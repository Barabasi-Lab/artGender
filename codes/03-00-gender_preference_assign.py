    ...
    def get_ins_gender(self):
        ...
        for preference_type, p_null in zip(["neutral", "balance"], [self.p_null_neutral, self.p_null_balance]):
            ...
            json.dump(gender_preference,
                      open(os.path.join(self.save_data_path, f"gender_{preference_type}_ins_bf10.json"), "w"),
                      indent=4)
            json.dump(info,
                      open(os.path.join(self.save_data_path, f"gender_{preference_type}_details_ins_bf10.json"), "w"),
                      indent=4)
    ...
    def get_country_gender(self):
        ...
        for preference_type, p_null in zip(["neutral", "balance"], [self.p_null_neutral, self.p_null_balance]):
            gender_preference, info = self.get_gender_preference(
                ...
                p_null)
            json.dump(gender_preference,
                      open(os.path.join(self.save_data_path, f"gender_{preference_type}_country_bf10.json"), "w"),
                      indent=4)
