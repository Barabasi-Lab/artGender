...
class PrepareSale:
    def __init__(self, genderizeio_threshold, remove_birth, career_start_threshold, preference_type,
        ...
        self.sales_select = pd.read_csv(os.path.join(self.save_data_path, "sales_select.csv"))
        self.agg = pd.read_csv(os.path.join(self.save_fig_path, f"artist_solo_exh_info_agg_{self.preference_type}.csv"))

        # create agg_sales
        self.agg_sales = self.create_agg_sales()
        self.sales_select = self.sales_select[self.sales_select["artist"].isin(self.agg["artist"])]
    ...
    def create_agg_sales(self):
        ...
        agg_sales.to_csv(os.path.join(self.save_data_path, f"artist_solo_sale_info_agg_{self.preference_type}.csv"),
                         index=False)
        return agg_sales
    ...
    def plot_population_imbalance(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, "e-number_auction_artist.pdf"))
    ...
    def plot_records_balance(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, "e-number_auction.pdf"))
    ...
    def plot_sales_imbalance(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, "e-total_sales_auction.pdf"))
    ...
    def plot_sold_pieces_imbalance(self):
        ...
        g.legend_.remove()
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, "e-auction_count_bias_barplot.pdf"))
    ...
    def plot_piece_price_imbalance(self):
        ...
        g.legend_.remove()
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, 'e-auction_price_bias_barplot.pdf'))
    ...
    def plot_access_rate(self):
        ...
        plt.gca().tick_params(axis='x', labelsize=25)
        plt.tight_layout()
        plt.savefig(os.path.join(self.save_fig_path, "e-access_rate.pdf"))
...
#
# print(len(artists_select))
# print(len(solo_shows_select), Counter(solo_shows_select["gender_recog"]))
...
# solo_shows_select["gender_preference"] = [int(gender_preference_dict[
#     str(ins)]) if str(ins) in gender_preference_dict else -1 for ins in solo_shows_select["institution"]]
#
# # create an agg_sales dataframe
# pieces_sold = sales_select.groupby("artist")["auction"].count(
# ).reset_index().rename(columns={"auction": "pieces_sold"})
# total_sales = sales_select.groupby("artist")["price real"].sum(
# ).reset_index().rename(columns={"price real": "total_sales"})
# avg_sales = sales_select.groupby("artist")["price real"].mean(
# ).reset_index().rename(columns={"price real": "avg_sales"})
# medium = sales_select.groupby("artist")["medium"].apply(list).reset_index()
# agg_sales = pieces_sold.join(total_sales.set_index("artist"), on="artist")
# agg_sales = agg_sales.join(avg_sales.set_index("artist"), on="artist")
# agg_sales = agg_sales.join(medium.set_index("artist"), on="artist")
# agg_sales = agg_sales.join(agg.set_index("artist"), on="artist")
# agg_sales.to_csv(
#     "../../../related_data_results/artist_sale_info_agg_solo_%s.csv" % preference_type)
