...
class AssignGenderPreference:
    ...
    def __init__(self, genderizeio_threshold, remove_birth, career_start_threshold, alpha=0.1 / 7719):
    ...
    def get_ins_gender(self):
        ...
        for preference_type, p_null in zip(["neutral", "balance"], [self.p_null_neutral, self.p_null_balance]):
            ...
            json.dump(gender_preference,
                      open(os.path.join(self.save_fig_path, f"gender_{preference_type}_ins_bonferroni.json"), "w"),
                      indent=4)
    ...
    def get_country_gender(self):
        ...
        for preference_type, p_null in zip(["neutral", "balance"], [self.p_null_neutral, self.p_null_balance]):
            gender_preference, power, filtered_female_count, filtered_total_count = self.get_gender_preference(
                ...
                self.alpha)
            json.dump(gender_preference,
                      open(os.path.join(self.save_fig_path, f"gender_{preference_type}_country_bonferroni.json"), "w"),
                      indent=4)
