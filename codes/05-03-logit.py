...
class Logit:
    def __init__(self, genderizeio_threshold, remove_birth, career_start_threshold, preference_type,
        ...
        self.auction_fig_path = os.path.join(base_figure_path, f"year_{self.career_start_threshold}",
                                             "auction")
        os.makedirs(self.auction_fig_path, exist_ok=True)
        ...
        self.artist_most_common_country_dict = self.get_most_country()
    ...
    def get_most_country(self):
        ...
        return artist_most_common_country_dict
    ...
    def create_logit_data(self):
        ...
        # add most common country/continent
        ...
        with sns.axes_style("whitegrid"):
            ...
            if self.save:
                in_auction_count_career[["career_length", "access_rate"]].to_csv(
                    "../main_paper_plot_data/4B_top.csv", index=False)
            in_auction_exhibition_count[["avg_exhibition_count", "access_rate"]].to_csv(
                "../main_paper_plot_data/4B_bottom.csv", index=False)
    ...
    def get_result_df(self, log_reg):
        ...
        rounded_conf_int_array = np.round(conf_int_array, 3)
        ...
        statistics_df = pd.DataFrame({
            ...
            'odds ratio': np.round(np.exp(log_reg.params.values), 3),
            'coef': np.round(log_reg.params.values, 3),
            'standard error': np.round(log_reg.bse, 3),
            'pvalues': [np.round(value, 3) if value >= 0.001 else "<0.001" for value in log_reg.pvalues],
    ...
    def create_result_summary_df(self, fitted_model_names, fitted_models):
        ...
        feature_names_pretty = ["Intercept", "Exhibition Count Per Year",
                                "Portion of Man-overrepresented Institution Exhibition",
                                "Portion of Woman-overrepresented Institution Exhibition",
                                "Career Length",
                                "Woman",
